...
The following code examples are taken from a VueJS project, using `axios` to call both the serveless functions and the APIs.
...
Make sure to test your code in the browser (remember to run `ntl dev` or `netlify dev` in the terminal to get both the Netlify local environment and your project up and running like you did in the [Run your build](#run-your-build) step)
...
You just tested the simplest function you could write and everything went smooth! To make a secure call to the API you want you'll first need to *read* the *API key* from you *environment variables*.
...
Now is time to create a new function called `almost-secret.js` that'll give you the value of `SECRET_API_KEY` env variable.
...
To make a HTTP call from a serverless function you need to import `axios` (you already installed the dependency in the previous steps).
...
The first line just import `axios` inside the function:
```javascript
import axios from 'axios';
```

After the common code for all the Netlify serverless function, you can spot the line taking care of retrieving the `QUIZ_API_TOKEN` env variable value:
```javascript
const { QUIZ_API_TOKEN } = process.env;
```
...
If the API respond with an error, the function will throw and exception (this is why everything is wrapped inside a `try/catch` block):
```javascript
if (response.errors !== undefined && response.errors.length) {
	throw new Error(response.errors);
}
...
```
In the response property we add the value of `response.data` since the endpoin return an array of strings that `axios` put right inside the `data` property (if you are familiar with `axios` you already know how to check this).
